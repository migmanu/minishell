...
/*   Updated: 2023/12/08 17:22:19 by jmigoya-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/minishell.h"

t_scmd	*redir_out_append(t_scmd *node, char **cmds, int *i, t_data *mish)
{
	int		flags[2];
	...
	flags[1] = 1;
	(*i)++;
	if (cmds[++(*i)] && (access(cmds[*i], F_OK) == 0
			|| (cmds[*i][0] != '>' && cmds[*i][0] != '|')))
		node->out_fd = get_fd(node->out_fd, cmds[*i], flags, mish);
	if (access(cmds[*i], F_OK) == -1 && (!cmds[*i] || cmds[*i][0] == '>'
		|| cmds[*i][0] == '|' || node->out_fd == -1))
	{
		if (cmds[*i] && cmds[*i][0] == '>')
...
t_scmd	*redir_in_heredoc(t_scmd *node, char **cmds, int *i, t_data *mish)
{
	char	*limit;
...
t_scmd	*redir_in(t_scmd *node, char **cmds, int *i, t_data *mish)
{
	...
	if (cmds[*i] && (access(cmds[*i], F_OK) == 0
			|| (cmds[*i][0] != '>' && cmds[*i][0] != '|')))
		node->in_fd = get_fd(node->in_fd, cmds[*i], flags, mish);
	if (access(cmds[*i], F_OK) == -1 && (!cmds[*i] || cmds[*i][0] == '>'
		|| cmds[*i][0] == '|' || node->in_fd == -1))
	{
